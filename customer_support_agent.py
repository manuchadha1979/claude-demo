...
def execute_tool(name: str, input_data: dict) -> ToolResult:
    ...
    if name == "get_ticket":
        ...
        return ToolResult(output=f"Fetched Freshdesk ticket {ticket_id}: 'Issue with login', status: Open")
        
    elif name == "lookup_crm_contact":
        email = input_data.get("emailid", "unknown@example.com")
        return ToolResult(output=f"Found HubSpot CRM contact: John Smith, ID 123, email: {email}")
        
    elif name == "query_order_history":
        return ToolResult(output="Supabase DB Query: Found 2 past orders. Order #9081 (Shipped), Order #4321 (Delivered)")
        
    elif name == "send_email":
        return ToolResult(output="SendGrid success: Resolution email sent successfully.")
        
    elif name == "post_slack_message":
        return ToolResult(output="Slack notification sent successfully to #escalations channel.")
...
tools = [
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "id": {"type": "string", "description": "customers ID"}
            },
            "required": ["id"]
        }
    },
